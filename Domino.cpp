			...
			if (!firstBone) {
				while (!playerHasValidBone) {
					...
					if (gameBoneYard.empty()) {
						...
						cout << "\n" << getCurrentPlayer().getName() << " There are no bones to play with and no more bones available for purchase!...\n\n";
						playerPassedTurn();
						utils::getUserInput();
						break;
					}
...
void Domino::addPlayersCLI() {
	...
	for (size_t i = 0; i < numPlayers; ++i) {
		...
		bool playerIsAI = utils::getYesNoCin("  >>> Do you want the player to be AI controlled (Y / N): "," Enter Y for the player to be AI controlled, or N if you want a human to control it! \n\n");
...
void Domino::showBoardAndPlayerStatus() {
	...
	cout << "Boneyard number of bones: " << gameBoneYard.getBonePile().size() << "\n";
...
void Domino::calcAndShowScoresOnRoundEnd(bool roundBlocked) {
	...
	cout << "\nTotal opponents points: " << bonus;
	cout << "\nRounded opponents total points: " << roundedBonus << endl;

	cout << "---------------------------------------------------------------------------------\n\n";

	cout << gamePlayers[winnerPlayerIndex].getName() << " won this round and got " << roundedBonus << " bonus points of your opponents!\n" << endl;

	cout << "*********************************************************************************\n";
	cout << "Scores: " << endl;
	for (size_t i = 0; i < gamePlayers.size(); ++i) {
		cout << " -> " << gamePlayers[i].getName() << " : " << (gamePlayers[i].getScore()) << ((gamePlayers[i].getScore() != 1) ? " score" : " points") << "\n";
	}
	cout << "*********************************************************************************\n";
...
bool Domino::loadDominoCLI() {
	utils::clearConsoleScreen();
	cout << "\n  >>> Enter the file name where the game you want to load is stored: ";
